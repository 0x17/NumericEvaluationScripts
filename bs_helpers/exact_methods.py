from bs_helpers import utils, common, validation, globals
import os
...
def convert_sm_to_json(fn):
    utils.syscall(utils.os_command_str('ConvertJson') + fn)
...
def solve_with_gams(solver, instance_name, trace=False, no_time_limit=False):
    ...
    s_time_limit = '9999999' if no_time_limit else str(globals.timelimit)
    s_iteration_limit = '999999' if globals.iterlimit == -1 else str(globals.iterlimit)
    num_threads = 0 if no_time_limit else 1
    outname = globals.outPath.replace('/', '')
    ...
    if not os.path.exists(globals.outPath): os.mkdir(globals.outPath)
    if os.path.exists(outname + 'myprofit.txt'):
        os.rename(outname + 'myprofit.txt', globals.outPath + 'myprofit.txt')
    if os.path.exists(outname + 'myschedule.txt'):
        os.rename(outname + 'myschedule.txt', globals.outPath + 'myschedule.txt')

    validation.validate_schedule_and_profit(instance_name, 'GMS_' + solver)

    os.rename('CPLEXTrace.txt', globals.outPath + 'CPLEXTrace_' + common.core_of_instance_name(instance_name) + '.txt')
...
def json_converter(fn, pfn, ctr, num_entries):
    convert_sm_to_json(fn)
