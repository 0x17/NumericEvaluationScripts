...
def generateGPlotCode():
    ...
    def isGamsTrace(fn):
        return inPrefixList(fn, GMS_PREFIXES)

    def mipMethod(fn):
        return inPrefixList(fn, MIP_PREFIXES)
    ...
    for fsentry in fsentries:
        ...
        if fsentry.endswith('Trace.txt'):
            ...
            plines += '\t\'' + fn + '\' using ' + str(colX) + ':' + str(
                colY) + ' lt rgb \'' + color + '\' dashtype 4 notitle smooth unique, \\\n'
            plines += '\t\'' + fn + '\' using ' + str(colX) + ':' + str(
                colY) + ' title \'' + title + '\' with points pointtype ' + str(
                ctr) + ' lt rgb \'' + color + '\' ps 0.6'
            if fsctr < entrycount:
                plines += ', \\\n'
...
def plotCurves(instname, termChoice, ext):
    ...
    os.system("gnuplot -e \"instTitle='" + instname.replace("_",
                                                            " ") + "'; outputFile='" + instname + "_" + termChoice + "_Trace." + ext + "'; terminalChoice='" + termChoice + "'\" plotSolverGenerated.gp")
