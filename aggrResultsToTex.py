...
NUM_METHODS = 14
REPR_NAMES = [ 'x_{jt}', '(\\lambda)', '(\\lambda|\\beta)', '(\\lambda|\\tau)', '(\\lambda|\\tilde{\\tau})', '(\\lambda|z_r)', '(\\lambda|z_{rt})' ]
SEP = ' & '
NA = ' - '
NEXTLINE = "\\\\ \n \\hline \n"
...
def decorate(v, inv=False):
	def col(c, s): return '\\textcolor{' + c + '}{' + s + '}'
	if v > 1.0: v = 1.0
	elif v < 0.0: v = 0.0
	vs = ("%.2f" % (v*100.0)) + '\\%'
	if inv:
		if v <= 0.1: return col('darkred', vs)
		elif v <= 0.4: return col('darkyellow', vs)
		else: return col('darkgreen', vs)
	else:
		if v <= 0.1: return col('darkgreen', vs)
		elif v <= 0.4: return col('darkyellow', vs)
		else: return col('darkred', vs)
...
def generateContent(title, sheet):
	...
	doinv = title == '\\%BKS'
	content += '$' + REPR_NAMES[0] + '$' + SEP + decorate(methNameToVal['GMS_Gurobi_'],doinv) + SEP + decorate(methNameToVal['LocalSolver'],doinv) + SEP + NA + SEP + NA + NEXTLINE
	for i in range(len(REPR_NAMES)-1):
		...
		content += '$'+REPR_NAMES[i+1]+'$' + SEP + NA + SEP + decorate(methNameToVal['LocalSolverNative' + str(mapLSIndex[i])], doinv) + SEP + gaStr + SEP + bbStr + NEXTLINE
